...
const Imoveis = () => {
	...
	return (
		<section className="w-full py-16 px-2 sm:px-4 md:px-8 section-dark">
			<div className="max-w-7xl mx-auto">
				<h2 className="section-title mb-10 text-center">Imóveis</h2>
                ...
                ${selected === cat.name
								? "btn-gold text-[var(--bg)]"
								: "bg-transparent text-[var(--gold)] border border-[var(--gold)] hover:bg-[var(--gold)] hover:text-[var(--bg)]"}
            `}
				...
				{selectedCategory && (
					<div id={selectedCategory.name.toLowerCase()} className="mb-16">
						<h3 className="text-2xl font-bold mb-6 flex items-center gap-2 section-title">
						...
						<div className="grid gap-8 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
							{selectedCategory.properties.map((property, pidx) => (
								<div
									key={pidx}
									className="card-glass overflow-hidden flex flex-col hover:scale-105 transition-transform"
								>
									<img
										src={property.image}
										alt={property.title}
									...
									<div className="p-4 flex-1 flex flex-col">
										...
										<p className="mb-4 flex-1">{property.description}</p>
										<a
											href="#contact"
											className="mt-auto btn-gold shadow transition"
										>
											Saiba mais
										</a>
									</div>
